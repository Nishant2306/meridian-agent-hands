...
| 14  | `--json` stdout has no log lines; caller output usable while evidence is pseudonymized | direct              | `integration/replay.cli.live` - exactly one JSON object on stdout with the real `Avery Lin` in it. **This row used to claim the human channel was covered too, and it was not**: every call passed `--json`, which suppresses that channel entirely, so the assertion held because there was no output. A second test now runs with `--json` OFF, checks stderr is populated before checking its content, and was mutation-tested. See D73 |
...
| The evidence commands work before one costs money             | direct              | `integration/evidence.sweep.live` runs the real replay sweep, the real fault-pinned boot and the REAL verifier against the tracked example, and is also the verifier's negative control - a bundle with no discovery in it must be REFUSED by name                       |
| The record identity is checked on the screen it is checked ON | direct              | `unit/agent.completion.identity` reproduces the GATE 3 failure from recorded observations, and asserts BOTH directions of why the obvious alternative would not have fixed it; `integration/agent.stale-identity.live` drives refusal and recovery through the real loop |
| A model that fixes the reason it was refused is told so       | direct              | `unit/agent.outstanding` covers the tracker including its SILENCE, which is half the design; the live test asserts the sentence reaches the evidence transcript                                                                                                          |
| Each key control resolves at the tier its evidence supports   | direct              | the same test asserts the verifier reports T1 for the search box, T3 for the table-labelled fields and T5 for the row control, and that nothing fell back to T4. Without this the fixture seed restart proves only that stable `name=` attributes exist                  |
...
it; `fixtures/legacy-app/tenants/tenant-b.ts` is a documented TODO that exports nothing. Until then
the surface-independence
...
`REPORT.md` carries the shorter table keyed to the brief's section 3, built from this one.

**One thing to check before that table is trusted.** The section numbers there are mapped onto the
seven report headings, which is the correspondence this build was driven against. If the brief's own
3.1 to 3.7 name something different, re-key the table rather than stretching a row to fit - a
requirement with no row here is **uncovered**, and saying so is worth more than a row that reads well.

The one already known to be uncovered is **multi-tenancy**. `semanticKey` exists on every descriptor,
nothing reads it, `fixtures/legacy-app/tenants/tenant-b.ts` exports nothing, and there is no test.
The `REPORT.md` row says that in those words.
