...
**This file is a TEMPLATE until somebody runs the commands below.** Every value written
`<<FILL AFTER RUN>>` is a run identifier or a result line that only a real run can produce. Nothing
in this repository invents one. A fabricated run id in a document that looks like a report is worse
than an empty template, because a reviewer cannot tell the two apart.
...
run id:      <<FILL AFTER RUN>>
result:      <<FILL AFTER RUN>>
llm calls:   <<FILL AFTER RUN>>
tiers used:  <<FILL AFTER RUN>>
```
...
run id:            <<FILL AFTER RUN>>
result:            <<FILL AFTER RUN>>
recoveries used:   <<FILL AFTER RUN>>
attempts on step:  <<FILL AFTER RUN>>
```
...
```
run id:    <<FILL AFTER RUN>>
error:     <<FILL AFTER RUN>>
expected:  <<FILL AFTER RUN>>
...
run id:            <<FILL AFTER RUN>>
error:             <<FILL AFTER RUN>>
steps completed:   <<FILL AFTER RUN>>
```
...
run id:            <<FILL AFTER RUN>>
result:            <<FILL AFTER RUN>>
completion mode:   <<FILL AFTER RUN>>
same session:      <<FILL AFTER RUN>>
interventions:     <<FILL AFTER RUN>>
```
