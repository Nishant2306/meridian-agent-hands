...
describe('one sign-on definition', () => {
  ...
  it('is referenced by BOTH the discovery CLI and the replay session broker', () => {
    ...
    for (const file of ['src/cli/discover.ts', 'src/replay/session-broker.ts']) {
      ...
      // NOTE: this string describes what the SOURCE file imports, not what this test imports.
      // Both `src/cli/discover.ts` and `src/replay/session-broker.ts` sit one level under `src/`,
      // so their path to the config module is `../config/sign-on.js`. When the tests moved into
      // subdirectories, a blanket relative-path rewrite turned this into `../../` and the
      // assertion started failing - which is the test catching a bad edit to itself.
