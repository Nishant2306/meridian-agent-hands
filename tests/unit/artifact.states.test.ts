...
describe('step rules, checked against real before/after observations', () => {
  ...
  it('[MUST] ACCEPTS an invariant that is true before AND after - that is what one is', () => {
    // The positive half of the rule above, and the one worth stating: an invariant is not a weaker
    // effect, it is a thing that does not change. `step8` carries the record-identity check, which
    // holds on the screen the step starts from and on the screen it ends on.
    const issues = checkStepDiscrimination(
      step8,
      before,
      after,
    ...
    expect(step8.invariants.length).toBeGreaterThan(0);
    expect(issues.map((issue) => issue.code)).not.toContain('INVARIANT_FALSE_BEFORE_ACTION');
    expect(issues.map((issue) => issue.code)).not.toContain('INVARIANT_FALSE_AFTER_ACTION');
    expect(issues.map((issue) => issue.code)).not.toContain('INVARIANT_IS_AN_EFFECT');
  });

  it('[MUST] rejects an invariant that does not survive its own transition', () => {
    // THE GATE 1 DEFECT, in a unit test. The distiller took the strong identity check from the FROM
    // screen - the cell beside "Member ID" - and that cell does not exist on the screen the step
    // ends on. It distilled cleanly, validated cleanly, and failed on the NEXT step, one transition
    // downstream of the thing that was actually wrong. See DECISIONS.md D30.
    const doesNotSurvive = {
      ...step8,
      invariants: [
        {
          id: 'from-the-screen-we-just-left',
          kind: 'text_present' as const,
          expected: { kind: 'literal' as const, value: 'Continue' },
          description: 'the Continue button, which exists on the form and not on the review screen',
        },
      ],
    };
    ...
    expect(issues.map((issue) => issue.code)).toContain('INVARIANT_FALSE_AFTER_ACTION');
  });

  it('rejects an invariant that was not true to begin with', () => {
    // The other side. An invariant false BEFORE the action means the step was never valid from
    // this state, and running it would be acting on an assumption nobody checked.
    const falseBefore = {
      ...step8,
      invariants: [
        {
          id: 'never-true-here',
          kind: 'text_present' as const,
          expected: { kind: 'literal' as const, value: 'Nothing on either screen says this' },
          description: 'an invariant that does not hold on the screen the step starts from',
        },
      ],
    };
    ...
    expect(issues.map((issue) => issue.code)).toContain('INVARIANT_FALSE_BEFORE_ACTION');
  });
