...
`docs/TEST_MAP.md` is the accurate per-claim version and names what is **thin** rather than padding a
row. **Multi-tenancy in 3.4 is uncovered**, not partly covered: there is no second tenant and no
test, and that row rests on the surface contract alone.

## 1. Architecture

Three modes over one machine. **Discovery** puts a model in an observe-decide-act loop against a live
UI; **distillation** turns the successful run into a typed capability; **replay** re-executes it with
no model in the loop. `src/agent` is the only package that imports a provider.
...
second path is the obvious way to make replay faster, and it is how you get a guardrail that holds in
discovery and not in production. A lint test fails the build on `page.click` or `page.goto` outside
...
class names change on every boot.
...
distiller rejects it; an invariant taken from the screen a step leaves distills cleanly and fails one
transition later, which is exactly how a real defect reached replay.
...
saying otherwise would be the easiest lie in this document.
...
interpreting second produces code that looks right and reports `MEMBER_NOT_FOUND` correctly, just
always at the cost of a full timeout. That is the difference between a service and a queue, so the
test asserts elapsed time against the step's own timeout.
...
`failed` (30) because "there is no such member" must not page anybody, and there is no
`RECORD_NOT_FOUND` error in the type system for a careless branch to conflate them through.
...
box at T1 by role and accessible name, the table-labelled fields at T3 by the adjacent label, the row
control at T5 structurally by its key cell. Without that the restart proves little - the fixture keeps
its legacy `name=` attributes, so a run that fell back to T4 everywhere would survive it while the
accessibility-first claim did no work.
...
and prints a console URL with the token on a separate line.
...
Resume is **anchor matching**, never "the furthest checkpoint that holds". Exactly one resume-eligible
state must match; zero or two go back to the person. A partly filled form matches nothing and returns
to the human, which is cheaper than typing over work somebody just did. Identity invariants are
checked first, so a wrong record is reported as one.
...
configurable engine on top. The effective decision is the stricter of the two, and both are tested
separately against the same action.
...
override. The capability prepares and stops at review. Prepare-don't-commit is the right shape for a
bank: it puts the model's work in front of a person at the point where reversal stops being cheap.

Three data mechanisms, never conflated. Persistence is **pseudonymized** with a per-run random map -
a truncated hash of a five-digit member id is enumerable in under a second and would look careful.
Artifacts are **scanned and rejected**, never rewritten, because a hit means the distiller has a bug
and rewriting hides it. Caller results are **not redacted**, because a capability that will not tell
its caller what it read is useless.
...
The console is loopback-only, the token is never in a URL, and there is no list-all endpoint: an
unknown id gets the same answer as a bad token.

## 7. Cuts

- **Desktop adapter.** First: the contract exists, the UI Automation work does not, and it is the
  claim most exposed to being wrong.
- **A second tenant**, to make surface-independence demonstrated rather than designed.
- **A second capability.** One cannot show whether the schema generalises.
